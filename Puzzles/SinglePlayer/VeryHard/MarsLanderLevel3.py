...
def read_surface_and_landing():
    ...
    for i in range(surface_n):
        ...
        surface.append(np.array([land_x, land_y]))
    ...
    return surface, landing_x, landing_y, landing_index


infinite = float("Inf")


def check_line_segments_cross(start1, end1, start2, end2):
    p = start1
    q = start2
    r = end1 - start1
    s = end2 - start2
    rs_cross = np.cross(r, s)
    if rs_cross == 0:
        return False  # Parallel (could be collinear and intersecting, but we ignore this rare case)
    t = np.cross(q - p, s) / rs_cross
    u = np.cross(q - p, r) / rs_cross
    if 0 <= t <= 1 and 0 <= u <= 1:
        return True  # Intersect at p+tr=q+us
    return False
...
def find_target(from_state, to_state, surface, landing_platform_index):
    # Optimally to_state would be the target, but maybe we intersect the surface somewhere and we do not want to collide
    start1 = from_state["pos"]
    end1 = to_state["pos"]
    cross_indices = []
    for i in range(len(surface) - 1):
        if i == landing_platform_index:
            continue # ignore landing platform intersection
        start2 = surface[i]
        end2 = surface[i + 1]
        if check_line_segments_cross(start1, end1, start2, end2):
            cross_indices.append(i)  # if there is one crossing there is another one as we are on the same side!
    debug("Intersections with surface indicies {}", cross_indices)
    if len(cross_indices) < 2:
        return to_state
    # Find highest point that is above every point between the two intersection elements
    max_y = 0
    min_x = infinite
    max_x = 0
    for i in range(min(cross_indices), max(cross_indices) + 2):
        max_y = max(max_y, surface[i][1])
        min_x = min(min_x, surface[i][0])
        max_x = max(max_x, surface[i][0])
    tar_state = copy_state(to_state)
    # Target with good delta above and to the left (if obstacle is on "right" side of platform)
    tar_state["pos"] = np.array([(min_x - 300 if min(cross_indices) > landing_platform_index else max_x + 300), max_y + 700])
    return tar_state

surface, landing_area_x, landing_area_y, landing_platform_index = read_surface_and_landing()

# game loop
debug("Starting game loop")
start_speed_x = None
emergency_x_break = False
landed_state = make_state(np.mean(landing_area_x), landing_area_y, 0, 0, 0, 0, 0)
while True:
    ...
    target_state = find_target(curr_state, landed_state, surface, landing_platform_index)
    ...
    if min(steps_to_target) < infinite:
        ...
        steps_to_break = steps_to_power_state + steps_to_break_state \
                       + estimate_steps_between_states(break_state, target_state, only_speed=True)
        ...
        if ((steps_to_break[0] >= steps_to_target[0] and (
                steps_without_changes[0] is infinite or abs(curr_state["speed"][0]) >= 15)) or
                (steps_to_break[1] >= steps_to_target[1] and (steps_without_changes[1] is infinite or abs(
                curr_state["speed"][1]) >= 35))):
            debug("NEED TO BREAK")
    ...
    if over_landing_area:
        ...
        if abs(speed_x) <= 20:
            ...
            if (abs(speed_x) <= 2 or speed_y >= 0 or abs(target_state["pos"][1] - curr_state["pos"][1]) <= 2 * abs(
                    speed_y) or
                    (steps_over_land_x > 0 and steps_over_land_x >= steps_to_land_y)):
                target_rot = 0
                target_thrust = 0
            else:
